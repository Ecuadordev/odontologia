...
class Panel extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('cosentimiento_model');
	...
	public function guardarTratamiento()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if (!is_null($insert)) {
				...
				$resp['success'] = true;
				$resp['redirect'] = 'tratamientos/panel';
			} else {
				$resp['success'] = false;
			}
	...
	{
		$this->form_validation->set_rules('para', '', 'required');
		...
		$this->form_validation->set_rules('procedimiento', '', 'required');
		if ($this->form_validation->run() == TRUE) {
			$data['codi_tra'] = $this->input->post('id_tratamiento');
			$data['codi_pac'] = $this->input->post('id_paciente');
			$data['fecha_con'] = date('Y-m-d');
			$data['para_con'] = $this->input->post('para');
			$data['servicio_con'] = $this->input->post('servicio');
			$data['tipo_con'] = $this->input->post('tipo');
			$data['codi_enf'] = $this->input->post('diagnostico');
			$data['id_procedimiento'] = $this->input->post('procedimiento');
			$insert = $this->modelgeneral->insertRegist('consentimiento', $data);
			$resp = [];
			if (!is_null($insert)) {
				$resp['success'] = true;
				$resp['redirect'] = 'tratamientos/panel/registrarConsentimiento/' . $data['codi_tra'];
			} else {
				$resp['success'] = false;
			}
	...
	function registrarConsentimiento($id)
	{
		$data['enfermedades'] = $this->modelgeneral->getTableWhere('enfermedad', ['esta_enf' => 'S']);
		$data['consentimientos'] = $this->cosentimiento_model->getConsentimientos($id);
		$data['procedimientos'] = $this->tratamientos_model->getProcedimientos($id);
		$tratamiento = $this->tratamientos_model->getTratamiento($id);
		$data['tratamiento'] = $tratamiento;
		$this->load->view('layouts/header');
		$this->load->view('layouts/aside');
		$this->load->view('admin/tratamientos/consentimientos/nuevo', $data);
		$this->load->view('layouts/footer');
	}

	function borrarConsentimiento($id)
	{
		$data = $this->modelgeneral->getTableWhereRow('consentimiento', ['codi_consen' => $id]);
		$this->modelgeneral->deleteRegist('consentimiento', ['codi_consen' => $id]);
		$resp['success'] = true;
		echo json_encode($resp);
	}

	function imprimirTratamiento($id)
	{
		$this->mpdf = new mPDF('utf-8', "A4");
		$data['tratamiento'] = $this->tratamientos_model->getTratamiento($id);
		$html = $this->load->view('admin/tratamientos/tratamientos/imprimir', $data, TRUE);
	...
	function imprimirConsentimiento($id)
	{
		...
		$data['consentimiento'] = $this->cosentimiento_model->getConsentimiento($id);
		$html = $this->load->view('admin/tratamientos/consentimientos/imprimir', $data, TRUE);
		...
		$this->mpdf->SetTitle('Consentimiento Informado');

		$this->mpdf->setHTMLHeader($htmlHeader);
		$this->mpdf->setHTMLFooter($htmlFooter);
		$this->mpdf->writeHTML($css, 1);
		$this->mpdf->writeHTML($html, 2);
		$this->mpdf->Output('Consentimiento-Informado', 'I');
	}
