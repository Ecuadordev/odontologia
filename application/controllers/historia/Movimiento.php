...
class Movimiento extends CI_Controller
{
	...
	function historia($id)
	{
		...
		$data['planes_diagnostico'] = $this->modelgeneral->getTableWhereRow('paciente_planes_diagnostico', ['codi_pac' => $id]);
	...
	public function guardarPacientePlanesDiagnostico()
	{
		$data['enfermedades'] = join(",", $this->input->post('planesEnfermedades'));
		$data['observaciones'] = $this->input->post('planesObservaciones');
		error_log("paciente" . $this->input->post('paciente'));
		$data['codi_pac'] = $this->input->post('paciente');
		$planes_diagnostico = $this->modelgeneral->getTableWhereRow('paciente_planes_diagnostico', ['codi_pac' => $data['codi_pac']]);

		if ($planes_diagnostico) {
			$where['codi_pac'] = $this->input->post('paciente');
			$result = $this->modelgeneral->editRegist('paciente_planes_diagnostico', $where, $data);
		} else {
			$result = $this->modelgeneral->insertRegist('paciente_planes_diagnostico', $data);
		}
